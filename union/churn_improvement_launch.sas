/*=============================================================
  이탈예측 개선 실험 (한 번에 실행)
  1) 90/60/30일 라벨 윈도우 각각 비교
  2) cutoff 직전 30일 구매 트렌드 피처 추가 (기본 vs 기본+트렌드)
  3) scale_pos_weight로 불균형 처리
  4) 5-fold 교차검증으로 AUC 평균±표준편차 계산 (표본이 적어서 필수)

  전제조건: proj.sales_with_disc, proj.customer_segments 존재
  (STAGE 1, STAGE 2 실행 완료 상태)

  [백그라운드 실행] SAS 세션이 끊겨도 분석은 서버에서 계속 돌아가도록
  subprocess.Popen(start_new_session=True) 방식 사용
  (stage5 PART B와 동일한 세션 격리 패턴)

  산출물:
  - /home/student/open/churn_improvement_results.csv (숫자 결과표)
  - /home/student/open/plots/churn_improvement.png (비교 차트)
  - /home/student/open/churn_improvement_progress.log (실시간 로그)
  - /home/student/open/churn_improvement_DONE.flag (완료 표시)
============================================================= */
...
feature_cols_base = ["Recency", "Frequency", "Monetary", "AvgOrderValue", "AvgShipping",
...
/* -------------------------------------------------------------
   2. 백그라운드로 던지기 - SAS 세션과 분리 (한 번만 실행)
------------------------------------------------------------- */
proc python;
submit;
import subprocess
...
with open(log_path, "w") as f:
    proc = subprocess.Popen(
        [sys.executable, "-u", "/home/student/open/churn_improvement.py"],
...
print("완료 여부는 churn_improvement_status_check.sas 를 따로 실행해서 확인하세요.")
endsubmit;
run;
